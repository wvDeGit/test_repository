...
class Active
{
     ...
     * @param $active_id
     */
    public static function goodsActiveAdd($goods_id,$promo,$active_id)
    {
     ...
     * @param $goods_id
     */
    public static function goodsActiveEdit($active_id,$promo,$goods_id)
    {
        $goodsObj = new IModel('goods');
        $old_goods_id = '';
        switch($promo)
        {
            case "groupon":
            {
                ...
                $data = $modelObj->getObj('id = '.$active_id,'goods_id');
                if($data)
                {
                    $old_goods_id = $data['goods_id'];
                }
            }
            break;
            ...
            case "time":
            {
                ...
                $data = $modelObj->getObj('id = '.$active_id,'`condition`');
                if($data)
                {
                    $old_goods_id = $data['condition'];
                }
            }
            break;
        ...
        if($old_goods_id)
        {
            ...
            $goodsObj->setData($dataArray);
            $goodsObj->update('id ='.$old_goods_id);
        }
     ...
     * @param $promo
     */
    public static function goodsActiveDel($where,$promo)
    {
        $goodsObj  = new IModel('goods');
        $goods_ids = array();
        switch($promo)
        {
            ...
            {
                $modelObj = new IModel('regiment');
                $data = $modelObj->query($where,'goods_id');
                if($data)
                {
            ...
            {
                $modelObj = new IModel('promotion');
                $data = $modelObj->query($where,'`condition`');
                if($data)
                {
