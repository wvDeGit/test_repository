...
class Order_Class
{
    ...
    {
        $newCoderNo = $unique_no = substr(base_convert(md5(uniqid(md5(microtime(true)),true)), 16, 10), 0, 8);
                ...
                case 'code':
                {
                    ...
                    foreach($orderGoodsList as $k => $v)
                    {
                        for($i = 0; $i < $v['goods_nums']; $i++)
                        {
                            ...
                            //向用户发送code
                            $smsContent = smsTemplate::goodsCode(array('{code}' => $code,'{name}'=>$goods_array['name']));
	...
	public static function sendDeliveryGoods($order_id,$order_goods_relation,$sendor = 'admin')
	{
    	...
    	if($orderRow['takeself'] > 0)
    	{
    		...
    		$takeRow= $takeDB->getObj('id = '.$orderRow['takeself']);
	    	$replaceData = array(
