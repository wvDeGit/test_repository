...
class Market extends IController implements adminAuthorization
{
	...
	function pro_speed_edit_act()
	{
		...
		{
			$where = 'id = '.$id;
            Active::goodsActiveEdit($id,'time',$condition);
			$proObj->update($where);
		}
		else
		{
            $insert_id = $proObj->add();
            Active::goodsActiveAdd($condition,'time',$insert_id);
		}
		$this->redirect('pro_speed_list');
	}
	...
	function pro_speed_del()
	{
		...
		if($id)
		{
			...
			$where .= ' and type = 1';
            Active::goodsActiveDel($where,'time');
			$propObj->del($where);
			$this->redirect('pro_speed_list');
		}
		else
	...
	function regiment_edit_act()
	{
		...
		{
			$where = 'id = '.$id;
            Active::goodsActiveEdit($id,'groupon',$goodsId);
			$regimentObj->update($where);
		}
		else
		{
            $insert_id = $regimentObj->add();
            Active::goodsActiveAdd($goodsId,'groupon',$insert_id);
		}
		$this->redirect('regiment_list');
	}
	...
	function regiment_del()
	{
		...
		if($id)
		{
			...
			$where = ' id in ('.$id.')';
            Active::goodsActiveDel($where,'groupon');
			$regObj->del($where);
			$this->redirect('regiment_list');
		}
		else
    ...
    function cost_point_edit_act()
    {
        ...
        if($id)
        {
            Active::goodsActiveEdit($id,'costpoint',$goods_id);
        ...
        else
        {
            ...
            $insert_id = $costPointObj->add();
            Active::goodsActiveAdd($goods_id,'costpoint',$insert_id);
        }
        $this->redirect('cost_point_list');
    }
    ...
    function cost_point_del()
    {
        ...
        if($id)
        {
            ...
            Active::goodsActiveDel($where,'costpoint');
